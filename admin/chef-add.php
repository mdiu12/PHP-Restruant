...
if(isset($_POST['form1'])) {
    ...
    if($path!='') {
        ...
        if( $ext!='jpg' && $ext!='png' && $ext!='jpeg' && $ext!='gif' ) {
            $valid = 0;
            $error_message .= 'You must have to upload jpg, jpeg, gif or png file for chef photo<br>';
        }
    } else {
    	$valid = 0;
        $error_message .= 'You must have to select a photo for chef photo<br>';
    }
	...
	if($valid == 1) {
		...
		$statement = $pdo->prepare("SHOW TABLE STATUS LIKE 'tbl_chef'");
		$statement->execute();
		...
		$final_name = 'chef-'.$ai_id.'.'.$ext;
        move_uploaded_file( $path_tmp, '../uploads/'.$final_name );

        $final_name1 = 'chef-banner-'.$ai_id.'.'.$ext1;
        move_uploaded_file( $path_tmp1, '../uploads/'.$final_name1 );

	
		$statement = $pdo->prepare("INSERT INTO tbl_chef (name,slug,designation_id,photo,banner,detail,facebook,twitter,linkedin,youtube,google_plus,instagram,flickr,phone,email,website,chef_order,status,meta_title,meta_keyword,meta_description) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
		...
		$success_message = 'Chef is added successfully!';
...
<section class="content-header">
	...
	<div class="content-header-right">
		<a href="chef.php" class="btn btn-primary btn-sm">View All</a>
	</div>
</section>
				...
				<div class="box box-info">
					<div class="box-body">
						...
						<div class="form-group">
							...
							<div class="col-sm-1">
								<input type="text" autocomplete="off" class="form-control" name="chef_order" value="<?php if(isset($_POST['chef_order'])){echo $_POST['chef_order'];} ?>">
							</div>
						</div>	        
				        <div class="form-group">
